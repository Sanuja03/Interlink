...
const FloatingAvailabilityBtn = () => {
  ...
  useEffect(() => {
    // Only fetch if we have a confirmed interviewer appUser
    if (appUser?.role === "interviewer") {
      fetchStatus();
    }
  }, [appUser]);  // re-runs when appUser changes

  const handleSubmitSuccess = () => setSubmitted(true);
  const handleClosePopup = () => setShowPopup(false);

  // Don't render until appUser is confirmed as interviewer
  if (!appUser || appUser.role !== "interviewer" || loading) return null;
    ...
    <>
      ...
      >
        <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22"
          ...
          {submitted && <polyline points="9 16 11 18 15 14" strokeWidth="2.5" />}
        </svg>
