...
public class CjobpostController {
    ...
    @GetMapping
    public ResponseEntity<List<CjobpostResponseDTO>> getJobs(
    ...
    @GetMapping("/{id}")
    public ResponseEntity<CjobpostResponseDTO> getJobById(@PathVariable Long id) {
        CjobpostResponseDTO job = service.getJobPostById(id);
        if (job == null) {
